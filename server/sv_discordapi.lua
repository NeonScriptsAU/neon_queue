...
-- Improved tableContains to handle array-like tables
function tableContains(tbl, val)
    for _, v in ipairs(tbl) do
        if tostring(v) == tostring(val) then -- Ensure both are strings
            return true
        end
    end
    return false
...
-- Print Guild Information on Resource Start
...
-- Fetch Discord Roles for a Player
function GetPlayerDiscordRoles(src, callback)
    ...
    for _, id in ipairs(GetPlayerIdentifiers(src)) do
        if string.find(id, "discord:") then
            identifier = string.sub(id, 9) -- Extract Discord ID
            break
        end
    end
